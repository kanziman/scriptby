...
function AddEditScriptForm({
  ...
  // isEdit 시 초기값 설정 (파일 업로드 후 전달된 값)
  useEffect(() => {
    if (isEdit && translatedLanguage && uploadFileName) {
      setSelectedLanguage(translatedLanguage);
      setFileName(uploadFileName);
    }
  }, [isEdit, translatedLanguage, uploadFileName]);
  ...
  function handleOnClick() {
    ...
    const scriptToUse = newScript || { id: scriptId, ...buildBaseScript() };
  ...
  return (
    <>
      ...
      <ButtonGroup>
        {isEdit ? (
          <Button
            disabled={!isAddButtonActive || isUpdating}
...
export default AddEditScriptForm;
